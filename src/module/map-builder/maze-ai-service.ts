...
export class MazeAIService {
  ...
  async materialize(
    ...
    // 1. Build raw AI template definitions
    interface RawDef {
      cells: boolean[][];
      roomType: string;
      aiWalls?: any[];
    }
    const rawMap = new Map<string, RawDef>();
    for (const tDef of result.templates) {
      rawMap.set(tDef.name, {
        cells: tDef.cells.map((row: any[]) => row.map((c: any) => !!c)),
        roomType: tDef.roomType || 'empty',
        aiWalls: tDef.walls,
      });
    }

    // 2. Build rotated cells grid for all placements (for neighbor lookups)
    ...
    const grid = new Map<string, ResolvedPos>();
    for (const p of result.layout.placements) {
      const raw = rawMap.get(p.templateName);
      if (!raw) continue;

      const unrotated: MapTemplate = {
        id: 'tmp', name: '', description: '',
        gridCols: N, gridRows: N, cellSize: MAP_CELL_SIZE,
        cells: raw.cells, walls: [],
      };
      const rotated = MapRotationHelper.rotateTemplate(unrotated, (p.rotation || 0) as MapRotation);

      grid.set(`${p.gridX},${p.gridY}`, {
        gx: p.gridX,
        gy: p.gridY,
        cells: rotated.cells,
    ...
    // 2b. Enforce port cells using the connectivity graph
    if (this._lastGraph) {
      for (const [key, pos] of grid) {
        const ports = this._lastGraph.ports.get(key);
        if (ports) {
          this._enforcePortCells(pos.cells, N, ports);
          this._ensureInternalConnectivity(pos.cells, N, ports);
        }
      }
      console.log('[MazeAIService] Port enforcement and internal connectivity complete');
    }

    // 3. Create per-placement templates with connectivity-aware walls
    const allTemplateIds = new Set<string>();
    const placementIdMap = new Map<string, string>();

    for (const p of result.layout.placements) {
      const pos = grid.get(`${p.gridX},${p.gridY}`);
      if (!pos) continue;
      const raw = rawMap.get(p.templateName);
      if (!raw) continue;
      ...
      // Build unrotated template, then rotate (handles both cells and AI walls)
      let walls: MapWallSegment[];
      if (raw.aiWalls && raw.aiWalls.length > 0) {
        const aiWallSegments: MapWallSegment[] = raw.aiWalls.map((w: any) => ({
        ...
        const unrotated: MapTemplate = {
          id, name: '', description: '',
          gridCols: N, gridRows: N, cellSize: MAP_CELL_SIZE,
          cells: raw.cells, walls: aiWallSegments,
        };
        const rotated = MapRotationHelper.rotateTemplate(unrotated, pos.rotation);
        walls = rotated.walls;
      } else {
        const tempTemplate: MapTemplate = {
          id, name: '', description: '',
          gridCols: N, gridRows: N, cellSize: MAP_CELL_SIZE,
          cells: pos.cells, walls: [],
        };
        walls = templateService.autoGenerateWalls(tempTemplate);
      }

      // Fix boundary walls: replace normal walls with doors/openings at connected edges
      walls = this._fixBoundaryWalls(walls, pos.cells, N, p.gridX, p.gridY, grid);

      const template: MapTemplate = {
        id,
        name: `[Maze] ${p.templateName} (${p.gridX},${p.gridY})`,
        description: `AI 迷宫 - ${pos.roomType}`,
        gridCols: N, gridRows: N, cellSize: MAP_CELL_SIZE,
        cells: pos.cells,
        walls,
        roomType: pos.roomType as RoomType,
      };

      await templateService.save(template);
      allTemplateIds.add(id);
      placementIdMap.set(`${p.gridX},${p.gridY}`, id);
    }

    // 4. Build blueprint — rotation already applied, so placements use rotation=0
    const placements: MazeBlueprintPlacement[] = result.layout.placements.map(p => ({
      gridX: p.gridX,
      gridY: p.gridY,
      templateId: placementIdMap.get(`${p.gridX},${p.gridY}`) || '',
      rotation: 0 as MapRotation,
    }));
    ...
    // 5. Global BFS connectivity check (safety net)
    if (this._lastGraph) {
      const globalOk = this._globalConnectivityCheck(blueprint, templateService);
      if (!globalOk) {
        console.warn('[MazeAIService] Global connectivity check found issues — attempting repair');
      }
    }
   ...
   */
  private _getPassageWallType(roomTypeA: string, roomTypeB: string): MapWallType {
    if (roomTypeA === 'corridor' && roomTypeB === 'corridor') return 'ethereal';
  ...
  validateConnectivity(
    ...
    for (const p of blueprint.placements) {
      const template = templateService.getById(p.templateId);
      ...
      for (const nb of neighbors) {
        ...
        const nTemplate = templateService.getById(neighbor.templateId);
        if (!nTemplate) continue;
