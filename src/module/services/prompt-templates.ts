...
// ============================================================
// Archetype (变体) Design Guidance
// ============================================================

/**
 * 变体设计专用指导
 */
export const ARCHETYPE_DESIGN_GUIDANCE = `
## PF2e 变体（Archetype）设计规则

### 变体结构
一个变体由以下部分组成：

1. **入门专长（Dedication Feat）**
   - 等级通常为2级（也可以是更高偶数级）
   - \`category\` 为 \`"class"\`
   - \`traits.value\` 必须包含 \`"archetype"\` 和 \`"dedication"\`
   - 赋予角色变体的核心身份和基础能力
   - 先决条件通常是属性值要求（如"力量14"）或技能要求（如"训练级运动"）
   - 入门专长通常是被动的，提供持续性的基础增强

2. **后续专长（Follow-up Feats）**
   - 等级从4级起，按偶数递增
   - \`category\` 为 \`"class"\` 或 \`"skill"\`
...
**画风统一性**：变体中的所有专长必须服务于同一个核心主题/画风。名称、描述措辞、效果类型都要风格一致。

**递进关系**：
- 入门专长建立基础（引入核心概念/机制）
- 中级专长扩展应用（新的使用方式、增强效果）
- 高级专长达到巅峰（强力终极能力、机制的完全绽放）
...
**平衡考量**：
- 入门专长不应过强，避免"一点入门即可"的问题
- 后续专长应让玩家有动力继续投入变体
- 高级专长可以很强但需要足够的投入（等级+先决条件）
`.trim();

/**
 * 机制深度指导（五级）
 */
export const MECHANISM_DEPTH_GUIDE = {
  noneGuide: `
**无独有机制模式**

本变体不需要设计独有机制。每个专长应该是独立的、自成体系的能力。
专长之间通过画风和主题统一，而非机制关联。

设计要点：
- 每个专长独立运作，不依赖其他专长的状态或资源
- 专长之间通过主题和叙事连接
- 可以有简单的先决条件链，但不是机制依赖
`.trim(),
...
设计1个简单的共享概念，作为变体的标志性元素。

适合的机制模式：
- **简单姿态**：入门引入一个姿态，后续专长在该姿态下获得不同效果
- **标记系统**：入门获得标记能力，后续专长以不同方式利用标记
- **持续状态**：入门赋予一种持续效果，后续专长增强或改变该效果

设计要点：
- 机制要一句话就能解释清楚
- 不需要复杂的资源计数
- 后续专长对机制的依赖应该是"锦上添花"而非"必须"
`.trim(),

  moderateGuide: `
**中度机制模式**

设计2-3个互动要素，形成有趣的战术循环。

适合的机制模式：
- **简单资源池**：积累资源、按需消耗，不同专长提供不同的充能和消耗方式
- **条件连锁**：满足条件A后触发效果B，不同专长提供不同的条件和效果
- **双态切换**：两种模式之间切换，不同专长在不同模式下提供不同能力
- **充能与爆发**：通过战斗行为积累能量，达到阈值后释放

设计要点：
- 机制要有清晰的"输入→输出"循环
- 入门专长建立机制基础，中级专长拓展使用方式
- 玩家需要做出有意义的战术选择
`.trim(),
...
适合的机制模式：
- **多层资源系统**：多种资源相互转化或组合产生不同效果
- **动态状态流转**：在多个状态之间根据条件流转，每个状态有独特能力
- **标记与引爆**：对目标施加标记，累积后以不同方式引爆
- **位置依赖系统**：效果根据战场位置关系变化

设计要点：
- 系统内部逻辑要自洽，避免死循环或无效状态
- 入门专长可以引入核心框架，但只开放部分功能
- 高级专长解锁系统的高级用法
- 虽然复杂但每个选择点都应该直觉化
`.trim(),
...
设计完整的子系统，相当于一个迷你职业特性。

适合的机制模式：
- **完整资源管理系统**：类似法师的法术位或吟游诗人的乐曲
- **多阶段状态机**：复杂的状态转换网络，不同路线有不同的最终形态
- **组合系统**：多种基本元素可以自由组合产生大量不同效果
- **风险对赌系统**：主动选择不同风险等级，高风险高回报

设计要点：
- 必须有清晰的规则文本，玩家能够理解整个系统
- 入门专长引入系统的最基础形态
- 随着专长链推进逐步解锁系统复杂度
- 最高级专长展现系统的完全形态
- 即使是极端复杂的系统，每个单独的规则也应该简单明了
`.trim(),

  getGuide(depth: 'none' | 'light' | 'moderate' | 'heavy' | 'extreme'): string {
    switch (depth) {
      case 'none': return this.noneGuide;
      case 'light': return this.lightGuide;
      case 'moderate': return this.moderateGuide;
      case 'heavy': return this.heavyGuide;
      case 'extreme': return this.extremeGuide;
    }
  }
};
