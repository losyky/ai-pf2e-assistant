...
export class MerchantGeneratorApp extends FormApplication {
  ...
  private resolveTraitSlug(input: string): string {
    ...
    for (const t of this.availableTraits) {
      if (t.value === lower) {
        console.log(`[MerchantGenerator] 特质转换（精确value）: "${input}" → "${t.value}"`);
        return t.value;
      }
    }
    ...
    for (const t of this.availableTraits) {
      if (t.label.toLowerCase() === lower) {
        console.log(`[MerchantGenerator] 特质转换（精确label）: "${input}" → "${t.value}" (label: "${t.label}")`);
        return t.value;
      }
    }
    ...
    for (const t of this.availableTraits) {
      if (t.label.toLowerCase().includes(lower) || lower.includes(t.label.toLowerCase())) {
        console.log(`[MerchantGenerator] 特质转换（模糊匹配）: "${input}" → "${t.value}" (label: "${t.label}")`);
        return t.value;
      }
    }
    
    console.warn(`[MerchantGenerator] ⚠️ 特质转换失败，使用原始输入: "${input}" (availableTraits数量: ${this.availableTraits.length})`);
    return lower;
  }
